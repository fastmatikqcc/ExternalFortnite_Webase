...
public:
    void Startup()
    {
        ...
        svr.Post("/enable_radaresp", [](const httplib::Request& req, httplib::Response& res) {
            std::cout << "radaresp enabled" << std::endl;
            enable_radaresp();
            res.set_content(R"({"status": "success"})", "application/json");
            });

        svr.Post("/enable_triggerbot", [](const httplib::Request& req, httplib::Response& res) {
            std::cout << "triggerbot enabled" << std::endl;
            enable_triggerbot();
            res.set_content(R"({"status": "success"})", "application/json");
            });

        svr.Post("/enable_off_screen_arrow", [](const httplib::Request& req, httplib::Response& res) {
            std::cout << "off_screen_arrow enabled" << std::endl;
            enable_off_screen_arrow();
            res.set_content(R"({"status": "success"})", "application/json");
            });
        ...
        std::cout << "Web server address : http://localhost:8080" << std::endl;
        ...
        std::cout << "Now Open Fortnite . . ." << std::endl;
        ...
        kraft::GET_PROCESS(L"FortniteClient-Win64-Shipping.exe");
        ...
        GameBase = kraft::GET_BASE_ADDRESS();
