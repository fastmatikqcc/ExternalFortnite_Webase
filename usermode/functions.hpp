...
namespace uee
{
	...
	public:
		...
		auto get_player_name(uintptr_t player_state) -> std::string
		{
			...
			kraft::read_physical(static_cast<PVOID>(ftext), buffer, length * sizeof(wchar_t));
		...
		camera_position_s UpdateCamera()
		{
		...
		{
			if (kraft::is_valid(player_state))
			{
				uintptr_t HabaneroComponent = read<uintptr_t>(player_state + 0x9f0);
				if (kraft::is_valid(HabaneroComponent))
